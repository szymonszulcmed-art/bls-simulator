...
export default function Home() {
  ...
  const [openBls, setOpenBls] = useState(false);
  ...
  <button
    onClick={() => setOpenBls(!openBls)}
    className="w-full text-left"
  >
    <div className="inline-block px-4 py-2 rounded-full text-sm font-bold bg-blue-700 text-white">
      Dostępne
    </div>

    <div className="flex items-center justify-between mt-6">
      <h3 className="text-2xl font-bold">BLS / PBLS</h3>
  ...
  {openBls && (
    <div className="mt-6 space-y-3">
      ...
      <Link
        href="/pbls-dziecko"
        className="block bg-blue-700 hover:bg-blue-800 text-white p-4 rounded-2xl font-bold text-center"
      >
        PBLS dziecko
      </Link>
      ...
      <Link
        href="/pbls-niemowle"
        className="block bg-blue-100 hover:bg-blue-200 text-blue-700 p-4 rounded-2xl font-bold text-center"
      >
        PBLS niemowlę
      </Link>
    </div>
